...
# Load raw ratings
df_raw = pd.read_csv(f"{DATA_PATH}/u.data", sep='\t', names=['userId','movieId','rating','ts'])

# Prepare Surprise dataset and split
reader = Reader(rating_scale=(1,5))
data = Dataset.load_from_df(df_raw[['userId','movieId','rating']], reader)
...
for name, m in models.items():
    ...
    if name == 'Popularity':
        ...
        precs, recs = [], []
        for u in user_true:
            recs_u = m.recommend(u, df_raw, 10)
        ...
        rd_train, rd_test = sk_train_test_split(df_raw, test_size=0.2, random_state=42)
...
def get_poster_url(title):
    """Fetch poster via TMDb or return a placeholder image URL."""
    try:
        if TMDB_API_KEY:
            resp = requests.get(TMDB_SEARCH_URL, params={'api_key': TMDB_API_KEY, 'query': title}, timeout=5)
            if resp.ok:
